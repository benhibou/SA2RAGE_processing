...
int 	main ( int argc, char ** argv )
{
	...
	numer = ( int * ) malloc ( readpt * sizeof ( long ) );
	denom = ( int * ) malloc ( readpt * sizeof ( long ) );
        sa2Rage = ( double * ) malloc ( readpt * sizeof ( double ) );
        for ( i = 0 ; i < 4 ; i++ ) {
	    ...
	    if ( j < 1 ) {
	       break;
            ...
            for ( j = 0 ; j < readpt ; j++ ) {
                ...
                for ( n = 0 ; n < ni ; n++ ) {
                    if ( n == 1 || n == 3 )
                       denom[ k + j] += ( l[n][j] * l[n][j] );
                }
                numer[k + j] = (l[0][j] * l[1][j] + l[2][j] * l[3][j])  ;
                sa2Rage[k + j] = ( numer[k + j] / (double) denom[k + j] );

            }
	    fwrite ( sa2Rage, sizeof ( double ), readpt, fp );
	}
	fclose ( fp );
	free ( ptr );
	free ( l );
