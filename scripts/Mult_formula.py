...
# Mult_formula.py
# F. Mult et Al., MRM 67 (2012) 1609-1619
...
if ( len ( sys.argv ) == 2 ) :
        ...
        if ( re.search ( "help$", cmdarg ) or re.match( '-h$', cmdarg ) ):
                print ( "Usage: Mult_formula.py -enne <n> -alpha1 <alpha1> -alpha2 <alpha2>" \
...
T1 = np.array([]);
T1 = np.arange(0.05, 5, 0.005)
B1 = np.array([]);
B1 = np.arange(0.05, 2.5, 0.005)
E1 = np.array([])
E1 = np.exp( -TER / T1 )
EA = np.array([])
EA = np.exp ( - TA / T1 )
...
Factor_1d = np.array([],[])
Factor_1d = 1 - E1 * np.cos ( alpha_1 * B1 )
Factor_2d = np.array([],[])
...
SA2RAGE = np.array([],[])
SA2RAGE = np.divide ( np.multiply ( GRE_I, GRE_II ), ( np.multiply ( GRE_I, GRE_I ) + \
